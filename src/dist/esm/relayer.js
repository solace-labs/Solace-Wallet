...
export const relayTransaction = async (data, payer, connection) => {
    ...
    const transaction = web3.Transaction.populate(web3.Message.from(Buffer.from(data.message, "base64")));
    transaction.addSignature(new web3.PublicKey(data.publicKey), Buffer.from(bs58.decode(data.signature)));
